...
'
    ...
    //  Insert
    subclassSeeds.forEach((group, index) =>
      const lccClass = lccClassRep.findOne(index + 1);

      group.forEach((subclassSeed) => {
        const lccSubclass = new LccSubclass();

        lccSubclass.uid = subclassSeed[0];
        lccSubclass.descr = subclassSeed[1];
  ...
  async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      'TRUNCATE TABLE `lcc_subclass`',
    );
    await queryRunner.query(
      'TRUNCATE TABLE `lcc_class`',
    );
  }
}
